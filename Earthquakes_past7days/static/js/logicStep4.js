...
// ### ADD EARTHQUAKE DATA AS AN OVERLAY(13.6.4) ### LOGICSTEP3.JS
// NOTE:
...
// ### ADD A LEGEND TO THE MAP ### LOGICSTEP4.JS 
// https://leafletjs.com/examples/choropleth/
    // ADD CODE FOR THE CUSTOOM LEGEND CONTROL AND PASTE IT BELOW THE L.GEOJSON() LAYER WHERE WE ADD THE EARTHQUAKE LAYER TO THE MAP, EARTHQUAKES.ADDTO(MAP)
    // Next, remove the argument "map" from the legend function
                // We're going to add a legend to the map with legend.onAdd
                // ### The legend will be added to a div element on the index.html file using the ### DomUtil utility function. ###
    // Next, we're going to add magnitudes array
                // We'll add a colors array that holds the colors for our magnitudes
    // The final piece is to edit the for loop
                // The for loop will add the color choices from our colors array as a small box for the color of earthquakes and place the text of the magnitude range next to the box
...
for (var i = 0; i < magnitudes.length; i++) {
  ...
  return div;
        ...
        //   Let's review what's happening in this for loop: we want 1) explicit range btw 2 numbers(1-5) or opened ended range for the last entry on the legeng (5+)
        ...
        // magnitudes[i] + (magnitudes[i + 1] ? "&ndash;" + magnitudes[i + 1] + "<br>" : "+")
                ...
                // magnitudes[i]                            handles low end of the range
                // +                                        is concatenation operator glueing the 2 strings together
                // ?                                        conditional operator acting like really condensed if statement. EXAMPLE: condition ? result_if_true : result_if_false                                      
                // (magnitudes[i + 1]                       conditional expression. condition before the question mark is the condition to be EVALUATED 
                // "&ndash;" + magnitudes[i + 1] + "<br>"   How the expression will eavaluate if condition is TRUE
                // "+"                                      How the expression will eavaluate if condition is FALSE
